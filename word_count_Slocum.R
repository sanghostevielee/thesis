...
input_text1 <- read.delim("./Slocum_1.txt", header = FALSE)
...
word_count1 %>%
  ...
  ggtitle("Top 15 Words, Slocum_1")

# Plot only top 15 digital-related keywords
digital_keywords1 <- c("animation", "software", "user", "printing", "exploration", "computer", "electronic", 
                       "interactive", "sound", "digital", "gis", "programming", "hardware", "internet", "multimedia")
digital_word_count1 <- filter(word_count1, word %in% digital_keywords1)
digital_word_count1 %>%
  mutate(word = fct_reorder(word, n)) %>%
  ggplot(aes(x=word, y=n)) +
  geom_bar(stat="identity", fill="#e99e85", alpha=.6, width=.4) +
  geom_text(aes(label = n), hjust = 1.2, vjust = 0.4, color = "#E75E36", size = 3) +
  coord_flip() +
  xlab("") +
  ylab("Occurrence") +
  ylim(0, 600) +
...
input_text2 <- read.delim("./Slocum_2.txt", header = FALSE)
...
word_count2 %>%
  ...
  ylim(0, 3300) +
  geom_point(size=1, color="orange") +
  ggtitle("Top 15 Words, Slocum_2")

# Plot only top 15 digital-related keywords
digital_keywords2 <- c("user", "animation", "printing", "software", "computer", "interactive", "digital", 
                       "exploration", "generalization", "virtual", "device", "sound", "web", "electronic", "gis")
...
input_text3 <- read.delim("./Slocum_3.txt", header = FALSE)
...
word_count3 %>%
  ...
  geom_point(size=1, color="orange") +
  ylim(0, 3300) +
  ggtitle("Top 15 Words, Slocum_3")

# Plot only top 15 digital-related keywords
digital_keywords3 <- c("user", "animation", "printing", "software", "web", "computer", "interactive", 
                       "uncertainty", "http", "virtual", "digital", "generalization", "device", "gis", "sound")
