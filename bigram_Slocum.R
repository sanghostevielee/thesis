...
#input_text <- read.csv("./Slocum_All.csv", header = TRUE, sep = ",")
input_text <- read.delim("./Slocum_1.txt", header = FALSE)
input_text <- as_tibble(input_text)

slocum_bigrams <- input_text %>%
...
bigrams_separated <- slocum_bigrams %>%
...
ggsave("test0.png", width = 20, height = 15, units = "cm", dpi = 300)




# [All Editions] Insert edition as a new column
bigram_counts_union1 <- bigram_counts_union %>%
  mutate(edition = "3rd ed.")
...
ggsave("test1.png", width = 30, height = 20, units = "cm", dpi = 300)
